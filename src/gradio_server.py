...
keywords_manager = SubscriptionManager(config.arxiv_keywords_file)
OPENAI_MODEL_LIST = ["gpt-4o", "gpt-4o-mini", "gpt-3.5-turbo"]
OLLAMA_MODEL_LIST = ["llama3.1:8b-instruct-q8_0", "gemma2:27b", "qwen2.5:32b"]
...
def generate_arxiv_report(model_type, model_name, keywords):
    config.llm_model_type = model_type
    ...
    # 定义一个函数，用于导出和生成指定时间范围内项目的进展报告
    raw_file_path = arxiv_client.export_top_articles(keywords)  # 导出原始数据文件路径
    report, report_file_path = report_generator.generate_arxiv_report(raw_file_path)  # 生成并获取报告内容及文件路径
...
def update_model_list(model_type):
    if model_type == "openai":
        return gr.Dropdown(choices=OPENAI_MODEL_LIST, label="选择模型")
    elif model_type == "ollama":
        return gr.Dropdown(choices=OLLAMA_MODEL_LIST, label="选择模型")
...
with gr.Blocks(title="GitHubSentinel") as demo:
    ...
    with gr.Tab("GitHub 项目进展"):
        ...
        # 创建 Dropdown 组件
        model_name = gr.Dropdown(choices=OPENAI_MODEL_LIST, label="选择模型")
    ...
    # 创建 Arxiv 研究进展 Tab
    with gr.Tab("Arxiv 研究进展"):
        gr.Markdown("## Arxiv 研究进展")  # 添加小标题
        ...
        # 创建订阅列表的 Dropdown 组件
        subscription_list = gr.Dropdown(keywords_manager.list_subscriptions(), label="关键词列表", info="已订阅文献检索关键词")
        ...
        # 将按钮点击事件与导出函数绑定
        button.click(generate_arxiv_report, inputs=[model_type, model_name, subscription_list], outputs=[markdown_output, file_output])
